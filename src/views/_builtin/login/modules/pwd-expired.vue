...
const title = computed(() => configStore.getTitle());
const logo = computed(() => configStore.getLogo());
</script>
...
<template>
  <div class="login pc">
    ...
    <NRow align="stretch" class="login-box">
      <NCol :xs="0" :sm="12" :md="13">
        <div class="login-left">
          <img class="login-left__img" src="@/assets/images/banner.png" alt="banner" />
        </div>
      </NCol>
      <NCol :xs="24" :sm="12" :md="11">
        <div class="login-right">
          <NTabs class="login-right__form">
            <template #extra>
              <span style="color: red">密码已过期，请修改密码</span>
            </template>
            <ATabPane key="1" title="密码修改">
              <span></span>
              <ModifyPassword />
            </ATabPane>
          </NTabs>
    ...
    <div v-if="!appStore.isMobile" class="footer">
      <div class="beian">
        <div class="below text">
          {{ configStore.getCopyright()
          }}{{
            configStore.getBeian()
              ? ` ·
                    ${configStore.getBeian()}`
              : ''
          }}
    ...
    <NRow align="stretch" class="login-box">
      <NCol :xs="24" :sm="12" :md="11">
        <div class="login-right">
          <NTabs class="login-right__form">
            <template #extra>
              <span class="color-red">密码已过期，请修改密码</span>
            </template>
            <NTabPane key="1" name="密码修改">
              <ModifyPassword />
            </NTabPane>
          </NTabs>
        </div>
