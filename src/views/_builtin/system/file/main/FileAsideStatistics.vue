...
<template>
  <section class="percent">
    <NSpace class="statistic-space" align="center" size="medium" fill>
      <template #split>
        <NDivider direction="vertical" />
      </template>
      <NStatistic class="statistic-item" title="存储量" :value="totalData.size" :value-style="statisticValueStyle">
        <template #suffix>&nbsp;{{ totalData.unit }}</template>
      </NStatistic>
      <NStatistic class="statistic-item" title="数量" :value="totalData.number" :value-style="statisticValueStyle" />
    </NSpace>
    <div v-if="chartData.length > 0">
      <NDivider />
      <VCharts :option="option" autoresize class="h-120px w-150px" />
    </div>
  </section>
</template>
