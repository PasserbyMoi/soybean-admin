  ...
  setup(props) {
    ...
    function move(e) {
      ...
      if (status.value && isEnd.value === false) {
        ...
        if (move_block_left >= barArea.value.offsetWidth - Number.parseInt(blockSize.value.width, 10) / 2 - 2) {
          move_block_left = barArea.value.offsetWidth - Number.parseInt(blockSize.value.width, 10) / 2 - 2;
        }
      ...
      if (status.value && isEnd.value === false) {
        ...
        const data = {
          ...
          pointJson: secretKey.value
            ? encryptByAes(JSON.stringify({ x: moveLeftDistance, y: 5.0 }), secretKey.value)
